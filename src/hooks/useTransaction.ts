...
const useTransaction = () => {
  ...
  const sendTransaction = async (contractAddress: string, message: JsonObject, tokenToSend?: TokenToSend[]) => {
    ...
    // const txLink = `https://explorer.nibiru.org/tx/${tx.transactionHash}`;

    // toast(() => (
    //   <span>
    //   Transaction Successful: <a href={ txLink } target = "_blank" rel = "noopener noreferrer" > <b>{ tx.transactionHash } < /b></a >
